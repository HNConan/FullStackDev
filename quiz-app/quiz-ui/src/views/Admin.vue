  ...
  <main>
    <div class="container-fluid">
      ...
      <div v-if="admin" class="text-center">
        ...
        <button @click="deleteAllQuestion()" class="btn m-3 btn-danger">Supprimer toutes les questions</button>
        <button @click="deleteAllParticipants()" class="btn m-3 btn-warning">Supprimer toutes les participations</button>
...
export default {
  ...
  methods: {
    ...
    deleteAllParticipants() {
      // Envoyer une requête API pour supprimer la question avec l'ID donné
      quizApiService.deleteAllParticipants()
        .then((response) => {
          // Supprimer la question de la liste
          console.log("Tous les participations ont été supprimées")
        })
        .catch((error) => {
          console.error(error);
        });
